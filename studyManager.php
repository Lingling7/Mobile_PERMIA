 <?php
	...
	if ($_SESSION['studyPhase'] == -1) {
		$query = "INSERT INTO P4MobileMnewsUserLog (StudyId, init_time) VALUES ('" . $_SESSION['studyId'] . "' , NOW())";
		mysql_query($query);
		$_SESSION['userId'] = mysql_insert_id();
		if($_SESSION['studyId'] == -1){
			$_SESSION['studyId'] = $_SESSION['userId'] % $NUM_STUDIES;
		}
		$query = "UPDATE P4MobileMnewsUserLog SET StudyId= '" .$_SESSION['studyId'] ."' WHERE user_id ='".$_SESSION['userId']."' ";
		mysql_query($query);
  }

	if ($_SESSION['studyId'] != -1 && $_SESSION['studyPhase'] != -1) {
		mysql_query("
		    UPDATE P4MobileMnewsTaskPerformance SET
				  task_end = NOW()
	...
	$result = mysql_query(
	    "SELECT * FROM P4MobileMnewsStudy WHERE study_id = " . $_SESSION['studyId'] .
  ...
  if (!mysql_num_rows($result)) {
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		mysql_query("
		    INSERT INTO P4MobileMnewsTaskPerformance
